...
%
%=============================================================================
%  CORRECTIONS APPLIED TO THE ORIGINAL gilbreathOLOP.m
%=============================================================================
%
...
for n = 1:length(R)
    ...
    figure(4)
    plot(phN(k), 20*log10(magN(k)), markerStyles{idx}, ...
         'MarkerFaceColor', markerFaces{idx}, 'MarkerSize', 7);
    grid on
    hold on
    ...
    iLo = max(1,        k - r);
    iHi = min(numel(w), k + r);
    plot(phN(iLo:iHi), 20*log10(magN(iLo:iHi)), ...
         [-360 0], [0 0], '-k', ...
         [-180 -180], [-50 50], '-k')
...
%         pilot handling-qualities analysis.
    ...
    plot(ax, phaseL, magL_dB, 'r-', 'LineWidth', 1.4, ...
         'HandleVisibility','off');
    text(ax, -180, M_dB + 1, sprintf('  M = %+g dB (NS bound)', M_dB), ...
         'Color', 'r', 'FontSize', 8, 'HandleVisibility','off');
