...
% Pull the underlying line handles created by nicholsplot.
% Two robustness tweaks:
%   1) findall (not findobj) — nicholsplot lines have HandleVisibility='off'
%   2) match by COLOR — marker letter is preserved less reliably than color
ax        = gca;
dataLines = findall(ax, 'Type','line', '-not', 'Marker','none');

hG_line = []; hN_line = []; hGN_line = [];
for k = 1:numel(dataLines)
    c = dataLines(k).Color;
    if     isequal(c, [0 0 1]),   hG_line  = dataLines(k);   % blue  -> G
    elseif isequal(c, [1 0 0]),   hN_line  = dataLines(k);   % red   -> N
    elseif isequal(c, [0 0 0]),   hGN_line = dataLines(k);   % black -> G.N
    end
end

% Force the markers we wanted (in case nicholsplot dropped them) and label.
set(hG_line,  'Marker','o', 'LineStyle','none', 'MarkerSize', 4, ...
    'DisplayName', 'Aircraft Linear OLTF  G(j\omega)');
set(hN_line,  'Marker','*', 'LineStyle','none', 'MarkerSize', 5, ...
    'DisplayName', 'Describing function  N(j\omega,A_{\delta_c})');
set(hGN_line, 'Marker','.', 'LineStyle','none', 'MarkerSize', 8, ...
