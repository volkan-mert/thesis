...
for z = 1:N
    ...
    xo = [deltao(z), phi2(z)];   % initial guess for fminsearch
    ...
    % Closed-loop -> open-loop describing function for Nichols plot
    NoMag(z) = 20*log10( A / sqrt(1 - 2*A*cos(dtr*phi) + A^2) );
    NoPh(z)  = dtr*phi - atan2(-A*sin(dtr*phi), 1 - A*cos(dtr*phi));

    % Restart from a known seed if the solver stalls; report first onset
    if fval(z) > 1e-4
        if isnan(onset_freq)
            onset_freq = w(z);
...
figure(1); clf                          % Nichols chart
plot(plin - 360, 20*log10(mlin), '-o', ...
     NoPh/dtr,   NoMag,           '-*');
grid on
axis([-300, -50, -20, 12])
xlabel('Open-loop phase [deg]');
ylabel('Open-loop magnitude [dB]');
title('Nichols chart: linear loop vs. rate-limited OLOP DF');
legend('Linear G_c G_{ac}', 'OLOP describing function', 'Location','best');

figure(2); clf                          % Convergence diagnostic
semilogx(w, fval);
grid on
xlabel('\omega [rad/s]');
...
%   phN   - phase in RADIANS    (0 below onset; -acos(pi/(2x)) far above)
    ...
    if x < 1
        ...
        phN  = 0;
    elseif x < 1.862
        magN = polyval([ 0.2908 -1.4396  1.9232  0.2230 ], x);  % cubic spline
        phN  = polyval([ 0.5280 -2.6213  3.5056 -1.4171 ], x);  % cubic spline
    else
        magN = 4 / (x*pi);
        phN  = -acos(pi/(2*x));
    end
