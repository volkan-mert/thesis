...
%   To ses changes: visdiff('gilbreathDF.m','gilbreathDF_original.m')
...
for z = 1:N
    ...
    magGc  = squeeze(mGc);   phGc  = squeeze(pGc);   % squeeze collapses the 1×1×1 outputs to scalars
    magGac = squeeze(mGac);  phGac = squeeze(pGac);  % squeeze collapses the 1×1×1 outputs to scalars
...
[mag_onset, phase_onset] = bode(sys_dol, onset_freq); 

figure(1); 

hold on

h = nicholsplot(sys_lin, 'b-o', sys_dol, 'r-*', opts);

% Crosshair at the critical point (kept out of the legend)
p = nicholsplot(frd(squeeze(freqresp(sys_dol, onset_freq)), onset_freq), 'mp', 'MarkerSize', 16);
% plot(phase_onset - 360, mag_onset + 1, 'p', 'MarkerSize', 8, 'MarkerFaceColor', 'r')
xline(-180, 'k--', 'HandleVisibility', 'off');
yline(   0, '--', 'HandleVisibility', 'off');
plot(-180, 0, 'r+', 'HandleVisibility', 'off');

legend('Linear G_c G_{ac}', 'OLOP Describing Function', 'Open-Loop Onset Point (OLOP)', 'Location','best');
