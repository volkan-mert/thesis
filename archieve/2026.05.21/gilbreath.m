...
%
%  Thesis
%  AFIT/GAE/ENY/01M-02duced oscillation
%
...
w   = logspace(-1, 2, n);           % The input frequency range
% qco = 1.1;                        % Original value given in Gilbreath's paper of the Stick input amplitude in percentage
qco = 1.1;                          % Stick Input Amplitude in Percentage
R   = 60;                           % Number of lines of the vector of the Rate Limit
...
% The Linear Closed Loop Transfer Function:  pcl = K*Gc / (1 + Gc*Gac)
pcl = K*Gc / (1 + Gc*Gac);
dtr = pi/180;
...
[magpcl, p0] = bode(pcl, w(1));          % Initial guesses of the magnitude and phases due to initial phase
deltao(1)    = qco * squeeze(magpcl);    % Initial guess of the magnitude in dB
phi2(1)      = squeeze(p0);              % Initial guess of the phase in degrees
...
for z = 1:N
    ...
    % Use an anonymous function to pass normal variables to the solver
    obj_fun = @(x) eqs(x, w(z), R, dtr, magGc, magGac, phGc, phGac, K, qco);
...
nicholsplot(sys_lin, 'b-o', sys_dol, 'r-*', opts); % Drawing Nichols Chart for Linear OLTF and Nonlinear OLTF with RLE.
...
% Plot 3: Stability Boundary Overlay
v1 = [-60 -90 -100 -120 -140 -160 -180];
v2 = [13.5 7.5 5.5 2.5 1.1 0 0];    
plot(v1, v2, 'k-', 'LineWidth', 2);

% Stable/Unstable Region Definitions
text(-150, 10, "Unstable Region: PIO OLOP Above");
text(-150, -10, "Stable Region: No PIO OLOP Below");
...
function f = eqs(x, freq, R, dtr, magGc, magGac, phGc, phGac, K, qco) % The function of the Harmonic Balance Residual (The HMB of Gilbreath).

    deltao = x(1);
    phi2   = x(2);

    [magN, phN] = dfunction(freq, R, deltao);   % Describing Function (magN: gain, phN: rad)

    re = deltao/magGc * cos(dtr*(phi2 - phGc)) + deltao*magGac*magN * cos(dtr*(phi2 + phGac) + phN) - K*qco;    % real part

    im = deltao/magGc * sin(dtr*(phi2 - phGc)) + deltao*magGac*magN * sin(dtr*(phi2 + phGac) + phN);            % imaginary part

    f = re^2 + im^2;

end

function [magN, phN] = dfunction(freq, rate, inamp) % Describing Function of the Rate Limiting Element (Hanke/Duda Piecewise Form).
