...
sys_ctrl = tf(5.21*conv([1 -57.36],conv([1 4.26],[1 .55])), ...
              conv([1 2*.442*22.85 22.85^2],conv([1 0],[1 1.16])));


%% 3. Longitudinal Dynamics of the Aircraft
% Aircraft dynamics used in the attached script.
sys_dyn = tf(-10.524*conv([1 1.562],conv([1 .038],[1 0])), ...
             conv([1 2*.212*.088 .088^2],conv([1 3.75],[1 -44])));
...
M_II = 0.2908*alpha_II.^3 ...
     - 1.4396*alpha_II.^2 ...
     + 1.9232*alpha_II ...
     + 0.223;

phi_II = 0.528*alpha_II.^3 ...
       - 2.6213*alpha_II.^2 ...
       + 3.5056*alpha_II ...
       - 1.4171;
...
valid_II = phi_required_deg <= 0 & ...
           phi_required_deg >= phi_II_deg(end);

if any(valid_II)

    % interp1 requires the phase vector in increasing order.
    alpha_req_II(valid_II) = interp1( ...
        phi_II_deg(end:-1:1), ...
        alpha_II(end:-1:1), ...
        phi_required_deg(valid_II), ...
        'linear');
...
valid_III = phi_required_deg <= phi_III_start_deg & ...
            phi_required_deg > -90;
...
for region_now = 2:3
    ...
    for k = 1:length(k_cross)
        ...
        if region_now == 2
            ...
            M_limit = 0.2908*alpha_limit^3 ...
                    - 1.4396*alpha_limit^2 ...
                    + 1.9232*alpha_limit ...
                    + 0.223;

            phi_limit = 0.528*alpha_limit^3 ...
                      - 2.6213*alpha_limit^2 ...
                      + 3.5056*alpha_limit ...
                      - 1.4171;
...
plot(real(minus_inv_N_I),imag(minus_inv_N_I), ...
     'ks','MarkerFaceColor','k','MarkerSize',7);

% Region II and Region III negative inverse describing functions.
plot(real(minus_inv_N_II),imag(minus_inv_N_II), ...
     '--','LineWidth',1.3);

plot(real(minus_inv_N_III),imag(minus_inv_N_III), ...
     ':','LineWidth',1.5);
...
if isempty(result)
    ...
    for k = 1:size(result,1)
        ...
        plot(result(k,7),result(k,8), ...
             'ko','MarkerFaceColor','k','MarkerSize',7);

        text(result(k,7),result(k,8), ...
             sprintf('  A_i = %.3f, \\omega = %.3f', ...
             result(k,1),result(k,2)));
    end
...
%% 12. Nichols Chart in Duda Fig. 8 Style
% Duda Fig. 8 shows only:
%   1) G(jw), the linear portion,
%   2) the negative inverse describing-function locus -1/N,
%   3) the intersection point.
%
% For this figure only, Kf is removed from the plotted linear portion.
% The limit-cycle calculations in the previous sections are NOT changed.

% Linear portion used only for the Duda-style Nichols figure
sys_fig8 = sys_ctrl * sys_dyn;

G_fig8 = squeeze(freqresp(sys_fig8,w));
G_fig8 = G_fig8(:).';

phase_G_fig8 = angle(G_fig8)*180/pi;
phase_G_fig8(phase_G_fig8 > 0) = phase_G_fig8(phase_G_fig8 > 0) - 360;

mag_G_fig8_dB = 20*log10(abs(G_fig8));


% Combine Region II and Region III into one -1/N curve
...
phase_inv = [phase_inv_II phase_inv_III];
mag_inv_dB = [mag_inv_II_dB mag_inv_III_dB];


% Find the intersection for the system used in this figure.
% The same phase/magnitude conditions are used:
%
%       G(jw) N(jw,Ai) = -1
%
phi_required_fig8 = -180 - phase_G_fig8;

res_fig8 = nan(size(w));
alpha_fig8 = nan(size(w));


% Region II
valid2 = phi_required_fig8 <= 0 & ...
         phi_required_fig8 >= phi_II_deg(end);

if any(valid2)

    alpha_fig8(valid2) = interp1( ...
        phi_II_deg(end:-1:1), ...
        alpha_II(end:-1:1), ...
        phi_required_fig8(valid2), ...
        'linear');
    ...
    res_fig8(valid2) = ...
        mag_G_fig8_dB(valid2) + 20*log10(M);
end
...
valid3 = phi_required_fig8 <= phi_III_start_deg & ...
         phi_required_fig8 > -90;

if any(valid3)

    phi_req = phi_required_fig8(valid3)*pi/180;
    ...
    res_fig8(valid3) = ...
        mag_G_fig8_dB(valid3) + 20*log10(M);
end


% Detect a zero-dB magnitude crossing
k_int = find( ...
    isfinite(res_fig8(1:end-1)) & ...
    isfinite(res_fig8(2:end)) & ...
    res_fig8(1:end-1).*res_fig8(2:end) <= 0, ...
    1,'first');


% Plot
figure('Name','Nichols Chart - Duda Fig.8 Style', ...
       'NumberTitle','off');

hold on;

% G(jw): solid black line
plot(phase_G_fig8,mag_G_fig8_dB, ...
     'k','LineWidth',1.2);

% -1/N: dashed black line
plot(phase_inv,mag_inv_dB, ...
     'k--','LineWidth',1.2);
...
% Mark the intersection, if one exists
if ~isempty(k_int)

    i1 = k_int;
    i2 = i1 + 1;

    % Simple interpolation in log frequency
    r1 = res_fig8(i1);
    r2 = res_fig8(i2);
    ...
    G_lc_fig8 = squeeze(freqresp(sys_fig8,w_lc_fig8));

    phase_lc = angle(G_lc_fig8)*180/pi;
    if phase_lc > 0
        phase_lc = phase_lc - 360;
    end

    mag_lc = 20*log10(abs(G_lc_fig8));

    plot(phase_lc,mag_lc, ...
         'ko','MarkerFaceColor','k','MarkerSize',6);

    text(phase_lc + 4,mag_lc, ...
        sprintf('limit cycle (%.2f rad/sec)',w_lc_fig8), ...
        'FontSize',11, ...
        'VerticalAlignment','middle');

    % Labels positioned in the same style as Duda Fig. 8
    text(phase_lc - 7,mag_lc - 2.3, ...
        'G(j\omega)','FontSize',11);

    text(phase_lc + 28,mag_lc + 4.5, ...
        '-1/N(j\omega,u_0)','FontSize',11);

else

    text(-145,5,'NO LIMIT CYCLE', ...
        'HorizontalAlignment','center', ...
        'FontSize',11);
...
figure('Name','MATLAB Nichols Plot of G(jw)', ...
       'NumberTitle','off');
