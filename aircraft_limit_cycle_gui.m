...
% =========================================================================
    ...
    xlim(ax_phase, [-10, 10]); ylim(ax_phase, [-20, 20]);
    ...
    xlim(ax_nichols, [-180, -50]); ylim(ax_nichols, [-5, 20]);
    ...
    while ishandle(fig)
        if is_running
            ...
            % -------------------------------------------------------------
            % Extract dynamic Ai from actual running actuator error
            ...
            if Ai < 0.1, Ai = 0.1; end % Prevent div by zero
    ...
    function run_sweep(~, ~)
        ...
        for i_tau = 1:length(tau_vec)
            ...
            for j_kp = 1:length(Kp_vec)
                % Break gracefully if the user clicks "Stop & Close" or closes the window during sweep
                if ~ishandle(fig)
