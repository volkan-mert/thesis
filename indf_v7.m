...
for k = 1:numel(u_rle)
    ...
    for m = 1:numel(real_int)
        ...
        % Annotate the first intersection of the selected -1/N curve.
        if ~annotated && abs(u_rle(k) - target_u) < 1e-12
            plot(ax, real_int(m), imag_int(m), 'ro', ...
                'MarkerSize', 13, 'LineWidth', 1.8);

            % Scale the text offset according to the current plot limits.
            drawnow;
            xl = xlim(ax);
            yl = ylim(ax);
            x_offset = 0.03 * max(diff(xl), eps);
            y_offset = 0.03 * max(diff(yl), eps);

            % Write the limit-cycle frequency next to the intersection.
            text(ax, real_int(m) + x_offset, imag_int(m) + y_offset, ...
