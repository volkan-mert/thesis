...
% 8. Eigenvalue with zero real part => non-hyperbolic equilibrium (HGE 7-9-23 adapted)
%    Note we always make it change from positive to negative (not needed,
%    but this was done in the original implementations).
if nneg == homds.nneg
% The signs of the eigenvalues are still all the same
% We check if it is either the stable or unstable part that is 
% closer to the imaginary axis to define a smooth test function.
  if real(D1(ind))<-real(D1(lambda1))
    res(8,1) = sign(real(D1(lambda1)))*real(D1(lambda1));
  else
    res(8,1) = sign(real(D1(ind)))*real(D1(ind));
  end
else
  %Below we rely on the index mu1/ind or lambda1 that will certainly be non-empty.
  if nneg < homds.nneg % One of the negative eigenvalues has turned positive
    res(8,1) = -sign(real(D1(lambda1)))*real(D1(lambda1)); 
  elseif nneg > homds.nneg % One of the positive eigenvalues has turned negative
    res(8,1) = -sign(real(D1(ind)))*real(D1(ind));
  end
end
