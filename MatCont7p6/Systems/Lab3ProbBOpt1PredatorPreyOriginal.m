function out = Lab3ProbBOpt1PredatorPreyOriginal
out{1} = @init;
out{2} = @fun_eval;
out{3} = [];
out{4} = [];
...
dydt=[kmrgd(1)-(kmrgd(1)*kmrgd(2))/(1+par_alpha*kmrgd(1));
-kmrgd(2)+(kmrgd(1)*kmrgd(2))/(1+par_alpha*kmrgd(1))-par_delta*kmrgd(2)*kmrgd(2);];

% --------------------------------------------------------------------------
function [tspan,y0,options] = init
handles = feval(Lab3ProbBOpt1PredatorPreyOriginal);
y0=[0,0];
