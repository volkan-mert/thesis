...
%If the bifurcation point is not with sufficient accuracy and issue a warning.
  if(isempty(index2)==1)
    index2 = find(abs(real(diag(D)))<1e-5 & sign(imag(diag(D)))==1);% Hopf part again
    if(isempty(index2)==1)
      debug('Neutral saddle\n');
      coef =[0,0,0];
      return;
    end
    sprintf('Located Codim 2 point has low accuracy. Consider recomputing.');
  end
  ...
  index2 = find(abs(real(diag(D)))<1e-6 & sign(imag(diag(D)))==-1);% hopf part.
  if(isempty(index2)==1)
    index2 = find(abs(real(diag(D)))<1e-5 & sign(imag(diag(D)))==1);% Hopf part again, warning already issued
  end
  qad0 = X(:,index1);
  qad1 = X(:,index2);
  p0 = qad0/(q0'*qad0);
