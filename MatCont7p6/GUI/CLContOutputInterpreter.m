...
classdef CLContOutputInterpreter
    ...
    methods
        ...
        function [index, omap] = doCycle(obj, settings, coordinates, parameters, ntst, ncol, dim, omap, numconfig, plotsel)
            ...
            if ntst == 0
                ...
                for index = 1:length(coordinates)
                    ...
                    plotsel.declareItem('coordinates', coordinates{index}, 'Max', sprintf('max(%s)', coordinates{index}), @(x, h, f, s, ind, i)x(index, ind));
                    plotsel.declareItem('coordinates', coordinates{index}, 'Min', sprintf('max(%s)', coordinates{index}), @(x, h, f, s, ind, i)x(index, ind));
                end
                ...
                if hasrenewal
                    ...
                    for index = 1:delay.dim_r
                        labels(index, :) = {delay.coordinates{delay.ind_r(index),1}, @(x, h, f, s, ind, i) delay.rhs_r(x(:, i),parameters.getValue(),parameters.getActive(),delay.ind_r(index),ntst,ncol)};
                        plotsel.declareSubCategory('reconstructed', delay.coordinates{delay.ind_r(index),1}, @(x, h, f, s, ind, i) delay.rhs_r(x(:, ind),parameters.getValue(),parameters.getActive(),delay.ind_r(index),ntst,ncol))
                    end
                ...
                if hasrenewal
                    ...
                    for index = 1:delay.dim_r
                        coordinate = delay.coordinates{delay.ind_r(index),1};
                        plotsel.declareSubCategory('reconstructed', coordinate);
                        
                        plotsel.declareItem('reconstructed', coordinate, 'Default', coordinate, @(x, h, f, s, ind, i) delay.rhs_r(x(:, ind),parameters.getValue(),parameters.getActive(),delay.ind_r(index),ntst,ncol), true);
                        plotsel.declareItem('reconstructed', coordinate, 'Min', sprintf('min(%s)', coordinate), @(x, h, f, s, ind, i) min(delay.rhs_r(x(:, ind),parameters.getValue(),parameters.getActive(),delay.ind_r(index),ntst,ncol)));
                        plotsel.declareItem('reconstructed', coordinate, 'Max', sprintf('max(%s)', coordinate), @(x, h, f, s, ind, i) max(delay.rhs_r(x(:, ind),parameters.getValue(),parameters.getActive(),delay.ind_r(index),ntst,ncol)));
                    end
                end
        ...
        function omap = doX(obj, settings, compbranch, coordinates, parameters, ntst, ncol, dim, omap, numconfig, plotsel)
            [index, omap] = doCycle(obj, settings, coordinates, parameters, ntst, ncol, dim, omap, numconfig, plotsel);
