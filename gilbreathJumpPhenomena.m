...
clear; clc ;close all
global w R Kp magGc magGac phGc phGac dtr qco z
...
num_Gc = [5.21, -273.7855, -1425.240306, -700.1952408];
den_Gc = [1, 21.3594, 545.553804, 605.6621, 0];

Gc = tf(num_Gc, den_Gc);

% Gac = tf(-10.524 * conv([1 1.562], conv([1 0.038], [1 0])), conv([1 2*0.212*0.088 0.088^2], conv([1 3.75], [1 -1.44])));

num_Gac = [-10.524, -16.8384, -0.62466254, 0];
den_Gac = [1, 2.347312, -5.30606528, -0.18359616, -0.0418176];

Gac = tf(num_Gac, den_Gac);

% Linear closed loop  pcl = Kp*Gc / (1 + Gc*Gac)
pcl = Kp*Gc / (1 + Gc*Gac);
dtr = pi/180;
...
function f = eqs(x)                             % The function of the harmonic balance residual
    global w R dtr magGc magGac phGc phGac Kp qco z
    deltao = x(1);
    phi2   = x(2);

    [magN, phN] = dfunction(w(z), R, deltao);   % magN: gain, phN: rad

    re = deltao/magGc * cos(dtr*(phi2 - phGc)) + deltao*magGac*magN * cos(dtr*(phi2 + phGac) + phN) - Kp*qco;
